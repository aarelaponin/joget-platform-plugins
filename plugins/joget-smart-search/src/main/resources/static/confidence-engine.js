...
(function(global) {
    ...
    function ConfidenceEngine(config) {
        ...
        // Registry mode: a name or id alone is sufficient (no location criteria required).
        this.simpleSearch = config.simpleSearch === true || String(config.simpleSearch) === 'true';
    ...
    ConfidenceEngine.prototype.validateCriteria = function(criteria) {
        ...
        // Registry simple-search mode: a name or partial id alone is enough — the
        // location requirement below is farmer-registry-specific.
        if (this.simpleSearch && (hasName || hasPartialId)) {
            return { valid: true, type: 'criteria_match', message: '✓ Ready to search', canSearch: true };
        }
