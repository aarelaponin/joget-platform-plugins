...
(function(global) {
    ...
    function getCriteriaTypes(config) {
        ...
        // Registry mode: only a partial-id extra criterion — no farmer location fields.
        if (config && (config.simpleSearch === true || String(config.simpleSearch) === 'true')) {
            return [
    ...
    function SearchInstance(id, container, config) {
        ...
        // Subject noun rendered in the UI — configurable so this element serves any
        // registry (taxpayer, citizen, business, farmer, ...), not just farmers.
        var _el = (config && config.entityLabel) ? String(config.entityLabel).trim() : 'farmer';
        this.entityLc = _el.toLowerCase();
        this.entityCap = this.entityLc.charAt(0).toUpperCase() + this.entityLc.slice(1);
    ...
    SearchInstance.prototype.renderRecentFarmersPanel = function() {
        ...
        html += '  <span class="fss-recent-title"><i class="fa fa-history"></i> Recent ' + this.entityCap + 's</span>';
    ...
    SearchInstance.prototype.render = function() {
        ...
        html += '  <span class="fss-title">Find ' + this.entityCap + '</span>';
    ...
    SearchInstance.prototype.executeInlineLookup = function(value, inputType) {
        ...
        this.apiCall('GET', endpoint, null, function(response) {
            ...
            if (response.success && response.farmers && response.farmers.length > 0) {
                ...
                self.showInlineError('No ' + self.entityLc + ' found');
            } else {
    ...
    SearchInstance.prototype.showAutoSelectNotification = function(farmer) {
        ...
        notification.innerHTML = '<i class="fa fa-check-circle"></i> <span>' + this.entityCap + ' found and selected: <strong>' +
    ...
    SearchInstance.prototype.buildNoResultsHtml = function(response) {
        ...
        html += '  <p>No ' + this.entityLc + 's found matching your criteria</p>';
    ...
    SearchInstance.prototype.clearSelection = function() {
        ...
        if (this.displayFieldText && !this.inlineSearch) {
            this.displayFieldText.innerHTML = '<span class="fss-placeholder-text">No ' + this.entityLc + ' selected</span>';
        }
