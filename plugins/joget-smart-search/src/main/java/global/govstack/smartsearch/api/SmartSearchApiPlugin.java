...
import global.govstack.smartsearch.service.FarmerSearchService.*;
import global.govstack.smartsearch.service.RegistrySearchService;
...
public class SmartSearchApiPlugin extends ApiPluginAbstract {
    ...
    public ApiResponse search(
        ...
        try {
            ...
            // Registry mode (config-driven): reuse this plugin for ANY registry, not just
            // farmers. When a registryTable is configured, route to the generic service and
            // return the SAME response contract the element renders.
            RegistrySearchService.RegistryConfig rc = getRegistryConfig();
            if (rc != null && rc.isConfigured()) {
                String term = firstNonEmpty(criteria.getNationalId(), criteria.getName(), criteria.getPartialId());
                List<Map<String, Object>> recs = new RegistrySearchService(rc).search(term, 20);
                return registryResponse(recs, startTime);
            }
    ...
    public ApiResponse searchByNationalId(
        ...
        try {
            ...
            RegistrySearchService.RegistryConfig rc = getRegistryConfig();
            if (rc != null && rc.isConfigured()) {
                List<Map<String, Object>> recs = new RegistrySearchService(rc).exactByIdentifier(nationalId);
                return registryResponse(recs, startTime);
            }
    ...
    /**
     * Read the generic-registry mapping from this API definition's plugin properties.
     * Returns null when no registryTable is configured (→ the plugin stays in farmer mode).
     */
    private RegistrySearchService.RegistryConfig getRegistryConfig() {
        String table = getPropertyString("registryTable");
        if (table == null || table.trim().isEmpty()) {
            return null;
        }
        RegistrySearchService.RegistryConfig rc = new RegistrySearchService.RegistryConfig();
        rc.table = table.trim();
        String idc = getPropertyString("idColumn");
        if (idc != null && !idc.trim().isEmpty()) rc.idColumn = idc.trim();
        rc.identifierColumn = trimOrNull(getPropertyString("identifierColumn"));
        rc.primaryColumn = trimOrNull(getPropertyString("primaryColumn"));
        rc.secondaryColumn = trimOrNull(getPropertyString("secondaryColumn"));
        rc.extraColumn1 = trimOrNull(getPropertyString("extraColumn1"));
        rc.extraColumn2 = trimOrNull(getPropertyString("extraColumn2"));
        String sc = getPropertyString("searchColumns");
        if (sc != null && !sc.trim().isEmpty()) {
            for (String s : sc.split(",")) {
                if (!s.trim().isEmpty()) rc.searchColumns.add(s.trim());
            }
        }
        return rc;
    }

    /** Serialise generic-registry records into the element's response contract. */
    private ApiResponse registryResponse(List<Map<String, Object>> records, long startTime) {
        Map<String, Object> response = new LinkedHashMap<>();
        response.put("success", true);
        response.put("resultType", records.isEmpty() ? "NO_RESULTS" : "CRITERIA_MATCH");
        response.put("totalCount", records.size());
        response.put("farmers", records);   // same key the SmartSearchElement JS reads
        response.put("searchTime", System.currentTimeMillis() - startTime);
        return new ApiResponse(200, new JSONObject(response));
    }
